...
export async function purchaseProduct(viewer: ViewerContext, input: unknown, request?: Request) {
  ...
  const statements = [
    ...
    db.prepare("INSERT INTO audit_logs (id,actor_user_id,action,target_type,target_id,summary,after_state,request_metadata) VALUES (?,?,?,?,?,?,?,?)").bind(secureId("audit"), buyer.id, "PURCHASE_COMPLETED", "Order", orderId, `Placed order ${orderCode}`, JSON.stringify({ productId: row.id, quantity, total, status }), requestSummary(request)),
    db.prepare("INSERT INTO notifications (id,user_id,type,title,body,resource_type,resource_id) VALUES (?,?,?,?,?,?,?)").bind(secureId("notification"), buyer.id, "ORDER", `ORDER ${orderCode}`, requiresApproval ? "Funds reserved. Awaiting operator review." : "Dead-drop manifest accepted; item added to inventory.", "Order", orderId),
  ];
  if (!requiresApproval) {
...
export async function processOrder(viewer: ViewerContext, input: unknown, request?: Request) {
  ...
  statements.push(db.prepare("INSERT INTO notifications (id,user_id,type,title,body,resource_type,resource_id) VALUES (?,?,?,?,?,?,?)").bind(secureId("notification"), order.user_id, "ORDER", `ORDER ${order.order_code}`, parsed.data.decision === "APPROVE" ? "Operator approved delivery." : "Operator rejected the order and returned reserved nuyen.", "Order", parsed.data.orderId));
