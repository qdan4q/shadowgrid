...
const exact = new Map<string, string>([
  ["Host authority overview", "Обзор управления хостом"],
  ["Every control executes on the server and writes audit history. Identity inspection is read-only and never acts as the selected runner.", "Каждый элемент управления выполняется на сервере и записывается в журнал аудита. Проверка личности доступна только для чтения и не действует от имени выбранного раннера."],
  ...
  ["Searchable host identities with server-owned role, balance, clearance and restriction state.", "Доступные для поиска личности хоста с серверным управлением ролями, счетами, допуском и ограничениями."],
  ["Account values", "Параметры доступа"],
  ["Inspect runner identity", "Проверка личности раннера"],
  ...
  ["Hide, reveal and inspect restricted host listings without deleting their order history.", "Скрывайте, раскрывайте и проверяйте закрытые лоты, не удаляя историю заказов."],
  ...
  ["Every balance mutation has before/after values, reason and actor. Ledger rows are immutable host records.", "Каждое изменение баланса содержит значения до и после, причину и исполнителя. Строки реестра — неизменяемые записи хоста."],
  ...
  ["Host broadcast control", "Управление трансляциями хоста"],
  ["Host configuration", "Настройка теневого хоста"],
  ["Configuration node reserved. Additional controls require root authorization.", "Узел конфигурации зарезервирован. Дополнительные элементы требуют корневого допуска."],
  ["Order queue clear", "Очередь заказов пуста"],
  ["No vendor is waiting for an operator decision.", "Ни один продавец не ждёт решения оператора."],
  ...
  ["Close", "Закрыть"],
  ["Unregistered item", "Оригинальный товар"],
  ...
  ["Registered identity", "Удостоверение личности"],
  ...
  ["A palm-sized relay shim for a covert Matrix run.", "Ретранслятор размером с ладонь для скрытного забега по Матрице."],
  ...
  ["Time-limited identity package with a shallow registry history.", "Ограниченный по времени пакет личности с неглубокой регистрационной историей."],
  ...
  ["Sealed field kit marked for trained campaign characters.", "Запечатанный полевой набор для обученных специалистов."],
  ...
  ["Compact emergency gel packets for campaign use.", "Компактные пакеты экстренного коагулянтного геля."],
  ["Two sealed vials with faint violet interference.", "Две запечатанные ампулы со слабым фиолетовым свечением."],
  ["A week of authenticated freight-routing credentials.", "Неделя подтверждённых учётных данных для грузовых маршрутов."],
  ["Two relays will rotate keys at 03:00 campaign time. If your deck reports certificate FROST-18, disconnect and message LATCHKEY.", "Два ретранслятора сменят ключи в 03:00 по местному времени хоста. Если ваша дека сообщает о сертификате FROST-18, отключитесь и напишите LATCHKEY."],
  ...
  ["Your watched host changed certificate twice. Possible interception or hostile relay rotation.", "Отслеживаемый хост дважды сменил сертификат. Возможен перехват или враждебная ротация ретрансляторов."],
  ...
  ["Campaign clock advanced", "Часы хоста синхронизированы"],
  ["Local campaign time is 04:17, 18 November 2080. Tacoma weather: hard rain.", "Местное время хоста: 04:17, 18 ноября 2080 года. Погода в Такоме: сильный дождь."],
  ...
  ["A compact weapon assembled from mismatched corporate lots and tuned by an independent gunsmith.", "Компактное оружие, собранное из несовпадающих корпоративных партий и настроенное независимым оружейником."],
  ...
  ["Lot numbers have been acid-etched away. Sold in campaign-scale bundles rather than real-world specifications.", "Номера партий вытравлены кислотой. Продаётся запечатанными комплектами по двадцать патронов."],
  ["A Matrix program with loud diagnostics and a habit of naming every process after extinct birds.", "Программа Матрицы с громкой диагностикой и привычкой называть каждый процесс в честь вымерших птиц."],
  ...
  ["Wraith ships it in a municipal survey case with its serial mesh scrubbed.", "Wraith отправляет его в кейсе муниципального инспектора. Серийная сетка на кейсе полностью зачищена."],
  ...
  ["Includes screening, implantation and two follow-up appointments.", "Включает обследование, имплантацию и два последующих приёма."],
  ...
  ["Prepared for ritual warding; each stick has a different mineral texture and astral resonance.", "Подготовлен для ритуальных оберегов; каждый брусок имеет собственную минеральную текстуру и астральный резонанс."],
  ...
  ["Useful only inside the campaign's Seattle grid and invalidated after first confirmed exposure.", "Действует только внутри сиэтлской Сетки и аннулируется после первого подтверждённого раскрытия."],
]);
